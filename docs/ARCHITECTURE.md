...
Die ersten Schreiboperationen sind bewusst klein: Profil-Anzeigename in `profile.sii` sowie `money_account` und `experience_points` in `game.sii`. In realen Saves werden Spielerwerte nicht global nach Feldnamen gesucht: `economy.bank` referenziert die Bank-Unit mit `money_account`, während `experience_points` und die Karriere-Skills direkt aus der eindeutigen `economy`-Unit gelesen und dort verändert werden. Gleichnamige Felder anderer Units, insbesondere von `driver_ai`, bleiben unangetastet.

Für Fahrzeugfunktionen wird zuerst die echte Spieler-Unit über `economy.player` bestimmt. Der aktive Zustand wird bevorzugt über die Save-Referenzkette `player.assigned_vehicles -> player_vehicles -> vehicle/trailer` aufgelöst. Fehlt diese Kette in einem älteren Save, wird kontrolliert auf `player.assigned_truck` / `player.assigned_trailer` zurückgefallen. Unit-IDs und Unit-Typen müssen in allen Fällen eindeutig validierbar sein. Truck-Reparatur setzt ausschließlich die bekannten Wear-Felder des aktiven Trucks sowie dessen nummerierte Wheel-Wear-Felder auf 0. Der Kraftstoffwert `fuel_relative` wird nur im aktiven Truck verändert und ist auf den Bereich 0..1 begrenzt. Trailer-Reparatur folgt der aktiven `slave_trailer`-Kette bis maximal 20 Units, erkennt Zyklen und verändert ausschließlich bekannte Trailer-/Chassis-/Wheel-Wear-Felder. Andere Trucks oder Trailer im Save bleiben unverändert.

Die Fahrzeugauswahl liest die Besitz-Arrays `trucks[n]` und `trailers[n]` ausschließlich aus der über `economy.player` aufgelösten Spieler-Unit. Jede referenzierte Unit muss existieren, den erwarteten Unit-Typ besitzen und darf im jeweiligen Besitz-Array nur einmal vorkommen.
...
Motor und Getriebe werden über die `accessories[n]`-Referenzen des aktiven Truck-Units aufgelöst. Jede Accessory-ID muss eindeutig sein; ihre referenzierte Unit wird ausschließlich über ein vorhandenes, gequotetes `data_path` klassifiziert. Genau eine Accessory muss das Pfadsegment `/engine/` und genau eine das Segment `/transmission/` enthalten. Mehrdeutige oder fehlende Zuordnungen führen fail-closed zum Abbruch. Neue Definition-Pfade müssen unter `/def/vehicle/truck/` liegen, auf `.sii` enden, ausschließlich kontrollierte Pfadzeichen enthalten und exakt der erwarteten Kategorie Engine oder Transmission entsprechen. Der Backend-Kern prüft damit Struktur und Kategorie. Für die Benutzeroberfläche wird zusätzlich ein konservativer Powertrain-Katalog direkt aus den bereits vorhandenen eigenen Trucks des ausgewählten Saves gebildet: Nur Engine-/Transmission-Pfade von Trucks mit demselben `/def/vehicle/truck/<modell>/`-Root wie der aktive Truck werden angeboten. Doppelte Pfade werden zusammengeführt, der aktuell verbaute Eintrag wird markiert und strukturell nicht auflösbare nichtaktive Trucks werden nicht als Auswahlquelle verwendet. Damit muss die UI keine unbekannten Pfade raten. Ein späterer Katalog aus installierten Game-/Mod-Archiven ist ausdrücklich kein Bestandteil dieser Sicherheitsgarantie, solange SCS-Archive und komponentenspezifische Kompatibilität nicht verifiziert geparst werden.
